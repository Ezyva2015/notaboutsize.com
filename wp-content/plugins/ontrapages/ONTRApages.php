...
class ONTRApages
{
	...
	{
		add_action( 'template_redirect', array ( 'ONTRApages', 'addOPContainerTemplate'), 10 );
		...
		add_action( 'wp_loaded', array( 'ONTRApages', 'addONTRApagesPostType' ) );
		add_filter( 'post_type_link', array ( 'ONTRApages', 'removeSlug'), 10, 3 );
		add_action( 'pre_get_posts', array ( 'ONTRApages', 'interceptRequestTrickery') );
	}
	...
	// Intercepts the request and resets the custom post type
